...
Offlinedatacollector is deployed on the following stack.
...
As documented you should create/import your Celery app in ``offlinedatacollector/__init__.py`` so that you
can run the worker via::

    celery -A offlinedatacollector worker

Additionally you will need to configure the project settings for Celery::

    # offlinedatacollector.settings.staging.py
    import os
    from offlinedatacollector.settings.base import *

    # Other settings would be here
    BROKER_URL = 'amqp://offlinedatacollector_staging:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/offlinedatacollector_staging' % os.environ

You will also need to add the ``BROKER_URL`` to the ``offlinedatacollector.settings.production`` so
...
RabbitMQ user will be named offlinedatacollector_<environment> and the vhost will be named offlinedatacollector_<environment>
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out offlinedatacollector.key 2048
  # Make copy of the key with the passphrase
  cp offlinedatacollector.key offlinedatacollector.key.secure
  # Remove any passphrase
  openssl rsa -in offlinedatacollector.secure -out offlinedatacollector.key
  # Generate signing request
  openssl req -new -key offlinedatacollector.key -out offlinedatacollector.csr
